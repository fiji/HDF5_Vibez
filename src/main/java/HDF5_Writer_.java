 ...
 **    $RCSfile$
 **   $Revision: 2672 $$Name$
 **       $Date: 2009-05-18 12:36:14 +0200 (Mo, 18 Mai 2009) $
 ...
 **
 **  $Log$
 **  modified  2011/03/16 15:34:52  kun
 **  -- write 4D data with element_size 
 **
 **
 **
 **
...
public class HDF5_Writer_ implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
		...
		if (imp.getOpenAsHyperStack() || imp.isHyperStack()) {
			...
			try {
				...
				if (splitChannels) {
					// parse format string
					String[] formatTokens = HDF5_GroupedVarnames
							.parseFormatString(formatString, "[0-9]+"); // dummy
																		// regexp
					...
					for (int f = 0; f < nFrames; f++) {
						...
						for (int c = 0; c < nChannels; c++) {
							String fullName = makeDataSetName(formatTokens, f,
									c);
							...
							// ensure group exists
							Group group = createGroupRecursive(groupName, null,
									outFile);
							// create data set
							...
							try {
								dataset = (Dataset) outFile.get(groupName + "/"
										+ dataSetName);
							} catch (Exception e) {
								dataset = null;
							}
							if (dataset == null) {
								try {
									dataset = outFile.createScalarDS(
											dataSetName, group, type,
											channelDims, maxdims, chunks, gzip,
											null);
								} catch (Exception err) {
							...
							if (nLevs == 1) {
								...
								int stackIndex = imp.getStackIndex(c + 1, 1,
										f + 1);
							...
							try {
								// get pixel sizes
								ij.measure.Calibration cal = imp
										.getCalibration();
								System.out
								...
								try {
									element_size_um = getAttribute(dataset,
											"element_size_um");
								} catch (Exception e) {
									element_size_um = null;
								}
								if (element_size_um == null) {
									element_size_um = new Attribute(
											"element_size_um", attrType,
											attrDims);
								}
			...
			try {
				...
				if (imgColorType == ImagePlus.GRAY8) {
					...
					System.out.println("   bit depth: " + imgColorDepth
							+ ", type: GRAY16");
					int typeSizeInByte = 2;
					type = new H5Datatype(Datatype.CLASS_INTEGER,
							typeSizeInByte, Datatype.NATIVE, Datatype.SIGN_NONE);
				} else if (imgColorType == ImagePlus.GRAY32) {
					System.out.println("   bit depth: " + imgColorDepth
							+ ", type: GRAY32");
					int typeSizeInByte = 4;
					type = new H5Datatype(Datatype.CLASS_FLOAT,
	...
	private static Group createGroupRecursive(String groupRelativName,
		...
		while (groupRelativName.charAt(groupRelativName.length() - 1) == '/') {
			// trim last slash
			groupRelativName = groupRelativName.substring(0,
					groupRelativName.length() - 2);
		}
	...
	private String makeDataSetName(String[] toks, int frame, int channel) {
		String dName = toks[0] + Integer.toString(frame) + toks[1]
