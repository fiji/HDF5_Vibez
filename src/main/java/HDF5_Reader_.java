 ...
 **    $RCSfile$
 **   $Revision: 3629 $$Name$
 **       $Date: 2010-05-10 13:57:51 +0200 (Mo, 10. Mai 2010) $
 **   Copyright: GPL $Author: schlacht $
 ** Description:
 **
 **
 **
 ...
 **
 **  $Log$
 **  201/03/16 15:34:52  kun
 **  -- small modification to set imp.setOpenAsHyperStack(true) before imp.show();
 **
 **
 **
 **************************************************************************/
...
public class HDF5_Reader_ implements PlugIn {
	public void run(String arg) {
		...
		try {
			 ...
			 *-------------------------------------------------------------------*/
			boolean groupVarsByName = Boolean.getBoolean(HDF5_Config
					.getDefaultValue("HDF5.groupVarsByName"));
			groupVarsByName = Prefs
					.get("HDF5.groupVarsByName", groupVarsByName);

			boolean showUnmatchedDataSetNames = Boolean.getBoolean(HDF5_Config
					.getDefaultValue("HDF5.showUnmatchedDataSetNames"));
			showUnmatchedDataSetNames = Prefs
					.get("HDF5.showUnmatchedDataSetNames",
							showUnmatchedDataSetNames);

			String groupVarsByNameFormatGroup = HDF5_Config
					.getDefaultValue("HDF5.groupVarsByNameFormatGroup");
			groupVarsByNameFormatGroup = Prefs.get(
					"HDF5.groupVarsByNameFormatGroup",
				...
				if (gr != null) {
					...
					if (tmp instanceof String) {
						...
						for (int i = 0; i < sTmp.length; i++)
							groupVarsByNameFormat = groupVarsByNameFormat
									+ sTmp[i];
					}
					...
					System.out.println("File has no format string for grouping"
							+ ", using default");
					groupVarsByNameFormat = HDF5_Config
							.getDefaultValue("HDF5.groupVarsByNameFormat");
					groupVarsByNameFormat = Prefs
							.get("HDF5.groupVarsByNameFormat",
									groupVarsByNameFormat);
				}
			} catch (Exception e) {
			...
			String dollarRegexpForGrouping = HDF5_Config
			...
			// }
				...
				groupedVarnames.parseVarNames(varNames, groupVarsByNameFormat,
						dollarRegexpForGrouping);
				...
				// get unmatched names
				List<String> unmatchedVarNames = groupedVarnames
						.getUnmatchedVarNames();
				if (showUnmatchedDataSetNames) {
					// add all unmatched data set names to the dialog
					String[] varSelections = new String[unmatchedVarNames
							.size()];
					boolean[] defaultValues = new boolean[unmatchedVarNames
							.size()];
					for (int i = 0; i < unmatchedVarNames.size(); i++) {
						Dataset var = (Dataset) inFile.get(unmatchedVarNames
								.get(i));
				...
				if (groupedVarnames.getNFrames() > 0 && loadGroupedVarNames)
					while (wrongFrameRange || wrongChannelRange) {
						...
						if (frameRangeToks.length == 1) {
							...
							try {
								System.out
										.println("frame range with skipFrameIndex=1");
								minFrameIndex = Integer
										.parseInt(frameRangeToks[0]);
								maxFrameIndex = Integer
										.parseInt(frameRangeToks[1]);
								wrongFrameRange = false;
							} catch (Exception e) {
								wrongFrameRange = true;
							}
							...
							try {
								System.out
										.println("frame range with skipFrameIndex");
								minFrameIndex = Integer
										.parseInt(frameRangeToks[0]);
								skipFrameIndex = Integer
										.parseInt(frameRangeToks[1]);
								maxFrameIndex = Integer
										.parseInt(frameRangeToks[2]);
								wrongFrameRange = false;
							} catch (Exception e) {
								wrongFrameRange = true;
							}
						...
						if (channelRangeToks.length == 1) {
							// single channel
							try {
								minChannelIndex = Integer
										.parseInt(channelRangeToks[0]);
							...
							try {
								minChannelIndex = Integer
										.parseInt(channelRangeToks[0]);
								maxChannelIndex = Integer
										.parseInt(channelRangeToks[1]);
								wrongChannelRange = false;
							} catch (Exception e) {
								wrongChannelRange = true;
							}
							...
							try {
								minChannelIndex = Integer
										.parseInt(channelRangeToks[0]);
								skipChannelIndex = Integer
										.parseInt(channelRangeToks[1]);
								maxChannelIndex = Integer
										.parseInt(channelRangeToks[2]);
								wrongChannelRange = false;
							} catch (Exception e) {
								wrongChannelRange = true;
							}
				...
				if (showUnmatchedDataSetNames) {
					...
					for (int i = 0; i < unmatchedVarNames.size(); i++) {
						...
						try {
							...
							System.out
									.println("The file does not contain a variable "
											+ "with name "
											+ "`"
											+ dsName
											+ "`!");
				...
				System.out.println("addcheckboxgroup with " + varList.size()
						+ " rows");
				gd.addCheckboxGroup(varList.size(), 1, varSelections,
						defaultValues);
				addScrollBars(gd);
				gd.showDialog();
			 ...
			 *-------------------------------------------------------------------*/
			if (groupVarsByName && groupedVarnames.getNFrames() > 0
					&& loadGroupedVarNames) {
				groupedVarnames.setFrameAndChannelRange(minFrameIndex,
						skipFrameIndex, maxFrameIndex, minChannelIndex,
						skipChannelIndex, maxChannelIndex);
				...
				try {
					...
					if (ds != null && ds instanceof Dataset) {
						...
						if (isSigned16Bit) {
							GenericDialog convDiag = new GenericDialog(
							...
							convDiag.addChoice(
									"Please select an conversion option:",
									convOptions, convOptions[0]);
							convDiag.showDialog();
						...
						rank = var.getRank();
						extent = var.getDims();
						Attribute elemsize_att = getAttribute(var,
								"element_size_um");

						System.out.println("!!!!!!!!!!element_size_um: "
								+ (elemsize_att));
						...
						if (elemsize_att == null) {
							...
							try {
								...
								ij.gui.MessageDialog errMsg = new ij.gui.MessageDialog(
										null, title, msg);
								elem_sizes[0] = 1;
								elem_sizes[1] = 1;
								elem_sizes[2] = 1;
				...
				if (rank == 3 || rank == 2) {
					...
					ImageStack stack = new ImageStack(nCols, nRows, nFrames
							* nSlices * nChannels);
						...
						for (int cIdx = 0; cIdx < nChannels; cIdx++) {
							...
							for (int lev = 0; lev < nSlices; ++lev) {
								...
								if (wholeDataset instanceof byte[]) {
									byte[] tmp = (byte[]) extractSubarray(
											wholeDataset, startIdx, numElements);
									stack.setPixels(tmp, imp.getStackIndex(
											cIdx + 1, lev + 1, fIdx + 1));
								} else if (wholeDataset instanceof short[]) {
									short[] tmp = (short[]) extractSubarray(
											wholeDataset, startIdx, numElements);
									stack.setPixels(tmp, imp.getStackIndex(
											cIdx + 1, lev + 1, fIdx + 1));
								} else if (wholeDataset instanceof int[]) {
									System.out
											.println("Datatype `int` is not supported. "
									...
									System.out
											.println("Datatype `long` is not supported. "
													+ "Skipping whole frame!");
									// long[] tmp = (long[])
									// extractSubarray(wholeDataset,
									// startIdx,
									...
									float[] tmp = (float[]) extractSubarray(
											wholeDataset, startIdx, numElements);
									stack.setPixels(tmp, imp.getStackIndex(
											cIdx + 1, lev + 1, fIdx + 1));
								} else if (wholeDataset instanceof double[]) {
									System.out
											.println("Datatype `double` is not supported. "
													+ "Converting whole frame to `float`!");
									float[] tmp = convertDoubleToFloat((double[]) extractSubarray(
											wholeDataset, startIdx, numElements));
									stack.setPixels(tmp, imp.getStackIndex(
											cIdx + 1, lev + 1, fIdx + 1));
								} else {
									...
									stack.setPixels(
											extractSubarray(wholeDataset,
													startIdx, numElements), imp
													.getStackIndex(cIdx + 1,
															lev + 1, fIdx + 1));
								}
							}
						}
					...
					imp = new ImagePlus(directory + name + ": "
							+ groupedVarnames.getFormatString(), stack);
					System.out.println("nChannels, nSlices, nFrames: " + nChannels + ","+ nSlices +"," + nFrames);
					imp.setDimensions(nChannels, nSlices, nFrames);

					if (nChannels > 1) {
						System.out
								.println("Creating composite hyperstack with "
			...
			for (int i = 0; i < varList.size(); ++i) {
				if (gd.getNextBoolean()) {
					...
					System.out.println("");
					IJ.showStatus("Reading Variable: " + var.getName() + " ("
							+ extent[0] + " slices)");
					...
					System.out
							.println("   Element-Size in um (level,row,col): "
					...
					if (datatype.getDatatypeClass() == Datatype.CLASS_INTEGER
							&& (datatype.getDatatypeSize() == 4 || datatype
									.getDatatypeSize() == 8)) {
						System.out.println("Datatype not supported by ImageJ");
						GenericDialog typeSelDiag = new GenericDialog(
								"Datatype Selection");
						typeSelDiag.addMessage("The datatype `"
								+ datatype.getDatatypeDescription()
								+ "` is not supported by ImageJ.\n\n");
						typeSelDiag
								.addMessage("Please select your wanted datatype.\n");
						...
						if (selection == 0) {
							...
							datatypeIfUnsupported = new H5Datatype(
									Datatype.CLASS_FLOAT, Datatype.NATIVE,
									Datatype.NATIVE, -1);
						}
						if (selection == 1) {
							...
							datatypeIfUnsupported = new H5Datatype(
									Datatype.CLASS_INTEGER, typeSizeInByte,
									Datatype.NATIVE, -1);
						}
					}
					...
					if (rank == 5 && extent[4] == 3) {
                      //======================================================
                      //  Load 4D color Hypervolume
                      //======================================================
						System.out
								.println("   Detected HyperVolume (type RGB).");
						...
						for (int volIDX = 0; volIDX < extent[0]; ++volIDX) {
							...
							for (int lev = 0; lev < extent[1]; ++lev) {
								...
								if (wholeDataset instanceof byte[]) {
									...
									if (datatypeIfUnsupported
											.getDatatypeClass() == Datatype.CLASS_FLOAT) {
										float[] tmp = convertInt32ToFloat((int[]) extractSubarray(
												wholeDataset, startIdx,
												numElements));
									...
									if (datatypeIfUnsupported
										...
										short[] tmp = convertInt32ToShort((int[]) extractSubarray(
												wholeDataset, startIdx,
												numElements));
										short[] rChannel = new short[(int) stackSize];
										short[] gChannel = new short[(int) stackSize];
									...
									if (datatypeIfUnsupported
											.getDatatypeClass() == Datatype.CLASS_FLOAT) {
										float[] tmp = convertInt64ToFloat((long[]) extractSubarray(
												wholeDataset, startIdx,
												numElements));
										float[] rChannel = new float[(int) stackSize];
										float[] gChannel = new float[(int) stackSize];
									...
									if (datatypeIfUnsupported
											.getDatatypeClass() == Datatype.CLASS_INTEGER) {
										short[] tmp = convertInt64ToShort((long[]) extractSubarray(
												wholeDataset, startIdx,
												numElements));
										short[] rChannel = new short[(int) stackSize];
										short[] gChannel = new short[(int) stackSize];
						...
						imp.getCalibration().pixelHeight = elem_sizes[1];
						imp.getCalibration().pixelWidth = elem_sizes[2];
						double minmax[] = getMinMax(wholeDataset, selected[0]*selected[1]*selected[2]*selected[3]*selected[4]);
						...
						if (extent[3] == 3) {
                          ...
                          //======================================================
							System.out
									.println("   Detected color Image (type RGB).");
							...
							imp.getCalibration().pixelHeight = elem_sizes[1];
							imp.getCalibration().pixelWidth = elem_sizes[2];
                            double[] minmax = getMinMax( wholeDataset, selected[0]*selected[1]*selected[2]*selected[3]);
                            imp.setDisplayRange(minmax[0],minmax[1]);
						...
						{
                          //======================================================
                          //  Load 4D gray data set (hypervolume)
                          //======================================================
							System.out
									.println("   Detected HyperVolume (type GREYSCALE).");
							...
							for (int volIDX = 0; volIDX < extent[0]; ++volIDX) {
								...
								for (int lev = 0; lev < extent[1]; ++lev) {
									...
									if (wholeDataset instanceof byte[]) {
										...
										int[] tmp = (int[]) extractSubarray(
												wholeDataset, startIdx,
												numElements);
										if (datatypeIfUnsupported
												.getDatatypeClass() == Datatype.CLASS_FLOAT) {
											stack.addSlice(null,
													convertInt32ToFloat(tmp));
										}
										if (datatypeIfUnsupported
												.getDatatypeClass() == Datatype.CLASS_INTEGER) {
											stack.addSlice(null,
													convertInt32ToShort(tmp));
										}
									} else if (wholeDataset instanceof long[]) {
										long[] tmp = (long[]) extractSubarray(
												wholeDataset, startIdx,
												numElements);
										if (datatypeIfUnsupported
												.getDatatypeClass() == Datatype.CLASS_FLOAT) {
											stack.addSlice(null,
													convertInt64ToFloat(tmp));
										}
										if (datatypeIfUnsupported
												.getDatatypeClass() == Datatype.CLASS_INTEGER) {
											stack.addSlice(null,
													convertInt64ToShort(tmp));
										}
									} else if (wholeDataset instanceof float[]) {
										float[] tmp = (float[]) extractSubarray(
												wholeDataset, startIdx,
                      ...
                      //======================================================
						...
						if (slice instanceof byte[]) {
							...
							for (int row = 0; row < extent[0]; ++row) {
								for (int col = 0; col < extent[1]; ++col) {
									int offsetRGB = (row * (int) extent[1] * 3)
											+ (col * 3);
						...
						imp.getCalibration().pixelHeight = elem_sizes[1];
						imp.getCalibration().pixelWidth = elem_sizes[2];
						double[] minmax = getMinMax( slice, selected[0]*selected[1]*selected[2]);
						imp.setDisplayRange(minmax[0],minmax[1]);
						imp.show();
						imp.updateStatusbarValue();
					} else if (rank == 3) {
                      //======================================================
                      //  Load 3D gray data set (image stack)
                      //======================================================
						...
						for (int lev = 0; lev < extent[0]; ++lev) {
							...
							if (wholeDataset instanceof byte[]) {
								...
								stack.addSlice(
										null,
										extractSubarray(wholeDataset, startIdx,
												numElements));
							}
						}
						IJ.showProgress(1.f);
						...
						imp.getCalibration().pixelHeight = elem_sizes[1];
						imp.getCalibration().pixelWidth = elem_sizes[2];
						double[] minmax = getMinMax( wholeDataset, selected[0]*selected[1]*selected[2]);
						imp.setDisplayRange(minmax[0],minmax[1]);
						imp.show();
						imp.updateStatusbarValue();
					} else if (rank == 2) {
                      //======================================================
                      //  Load 2d gray data set (single image)
                      //======================================================
						// check if we have an unsupported datatype
						if (datatype.getDatatypeClass() == Datatype.CLASS_INTEGER
								&& (datatype.getDatatypeSize() == 4 || datatype
										.getDatatypeSize() == 8)) {
							System.out
									.println("Datatype not supported by ImageJ");
							GenericDialog typeSelDiag = new GenericDialog(
									"Datatype Selection");
							typeSelDiag.addMessage("The datatype `"
									+ datatype.getDatatypeDescription()
						...
						System.out.println("   Min = " + imgMin + ", Max = "
								+ imgMax);
						...
						System.err.println("   Error: Variable Dimensions "
								+ rank + " not supported (yet).");
						IJ.showStatus("Variable Dimension " + rank
								+ " not supported");
					}
				}
			}
			...
			System.err
					.println("Error while opening '" + directory + name + "'");
		...
		try {
			...
			System.err
					.println("Error while closing '" + directory + name + "'");
			System.err.println(err);
	...
	private static Attribute getAttribute(Dataset ds, String attrName)
			throws Exception {
	...
	private static Attribute getAttribute(HObject ds, String attrName)
			throws Exception {
...
/*
			...
			for (int i = 1; i < stackSize; i++) {
				...
				if (tmp[i] < 0)
					val = (float) Byte.MAX_VALUE - (float) Byte.MIN_VALUE
							+ (float) tmp[i] + 1;
...
*/
			...
			for (int i = 1; i < stackSize; i++) {
				...
				// we only support unsigned
				if (tmp[i] < 0)
					val = (float) Short.MAX_VALUE - (float) Short.MIN_VALUE
							+ (float) tmp[i] + 1;
